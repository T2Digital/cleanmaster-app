...
const AdminPage: React.FC = () => {
    ...
    const handleLogout = async () => {
        try {
            ...
            appContext?.showMessage('حدث خطأ أثناء تسجيل الخروج', 'error');
        }
    };
                    ...
                    <div className="max-w-md mx-auto">
                        <div className="bg-[#FCFCF9] p-8 rounded-lg shadow-md border border-[#5E5240]/[0.12]">
                            ...
                            <form onSubmit={handleLogin}>
                                <div className="mb-4">
                                    <label className="block mb-2 font-medium text-sm">البريد الإلكتروني</label>
                                    <input type="email" value={email} onChange={e => setEmail(e.target.value)} className="w-full px-4 py-2 border border-[#5E5240]/[0.2] rounded-lg bg-[#FCFCF9] focus:border-[#21808D] focus:ring-2 focus:ring-[#21808D]/50 outline-none" required />
                                </div>
                                <div className="mb-6">
    ...
    const totalRevenue = bookings.filter(b => b.status === 'completed').reduce((sum, b) => sum + (b.finalPrice || 0), 0);
    ...
    return (
        <div className={`bg-[#FCFCF9] p-4 rounded-lg shadow-sm border ${booking.status === 'new' ? 'border-[#21808D] ring-1 ring-[#21808D]/20' : 'border-[#5E5240]/[0.12]'}`}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm mb-3">
                ...
                <span><strong>التكلفة:</strong> {(booking.finalPrice || 0).toLocaleString()} جنيه</span>
